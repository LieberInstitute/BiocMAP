...
* *arioc_queue*: when running [INSERT PIPELINE NAME HERE] on an SGE/SLURM-based cluster, it is assumed GPU resources are available via a "queue" on the cluster. This variable must be set by the user to the queue name, in order to properly run [INSERT PIPELINE NAME HERE].
...
* *gpu_perc_usage_cutoff*: a percentage, specified as an integer, of GPU utilization (as measured by `nvidia-smi`) below which a GPU is considered "unoccupied" and thus available to use during alignment with Arioc.
