...
The `colData` slot of each `bsseq` object includes metrics collected from various processing steps throughout the pipeline. These metrics are also included in a standalone RDA file, documented below.

#### Metrics

One of the major pipeline outputs is an RDA containing an R data frame whose columns are different quality metrics, and whose rows are associated with sample names. A list of the exact column names and their descriptions is given below.
