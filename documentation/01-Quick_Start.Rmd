...
A brief guide to setting up BiocMAP. A more detailed and thorough guide is [here](#setup-details).
...
3. Run the command `bash install_software.sh [option]` to set up test files and software dependencies, where `[option]` must be replaced by `docker`, `conda`, `local`, or `jhpce`. These respectively (and in descending order of our recommendation) correspond to using docker to manage software dependencies, using conda, installing all software locally, or preparing BiocMAP for execution at the [JHPCE](https://jhpce.jhu.edu/) cluster.
