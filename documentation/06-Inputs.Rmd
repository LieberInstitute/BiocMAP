...
The second module requires the same `samples.manifest` file and an additional file called `rules.txt`. The latter specifies where relevant logs and the `samples.manifest` file are located. Similarly as in the first module, one must add the `--input` [argument](#command-opts) to [the approriate execution script for the second module](#quick-config), but its value should be the path to the directory containing `rules.txt`.
