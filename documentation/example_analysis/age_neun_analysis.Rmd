...
We'll briefly look at how the data is distributed by age and cell type shown in Table \@ref(tab:cell_types) and Table \@ref(tab:age_bins), respectively.

```{r metaTables}
cell_type_df = data.frame(table(meta$Cell.Type))
colnames(cell_type_df) = c("Cell Type", "Num Samples")
knitr::kable(
    cell_type_df, caption = "Distribution of cell type across samples",
    label = "cell_types"
)

age_bin_df = data.frame(table(meta$Cell.Type))
colnames(age_bin_df) = c("Age Bin", "Num Samples")
knitr::kable(
    age_bin_df, caption = "Distribution of age across samples",
    label = "age_bins"
)
```
