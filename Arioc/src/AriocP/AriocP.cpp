...
#pragma region C-style static variable definitions
static bool tsInit = AppGlobalCommon::SaveBuildTimestamp( __TIMESTAMP__ );  // (see AppGlobalCommon::SaveBuildTimestamp)
#pragma endregion
...
{
    AppGlobalCommon::SaveBuildTimestamp( __TIMESTAMP__ );
...
void AriocP::Main()
{
    ...
    AriocBase::aam.ms.SniffQ = m_hrt.GetElapsed( false ) - AriocBase::aam.ms.SniffQ;

    // load the R (reference sequence) and the H and J (lookup table) data into buffers
    loadR();
    loadHJ();

    // performance metrics
    CDPrint( cdpCD2, "%s: BuffersPerBinding=%d ThreadsPerGPU=%d", __FUNCTION__, OutputFileBinding::BuffersPerBinding, OutputFileBinding::ThreadsPerGPU );

    // initialize synchronization objects
    this->semLoadLUTs.Reset( this->nGPUs );
    this->semMainLoop.Reset( this->nGPUs );
    this->semUnloadLUTs.Reset( this->nGPUs );
