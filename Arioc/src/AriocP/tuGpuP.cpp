...
void tuGpuP::main()
{
    ...
    try
    {
        ...
        // load R sequence data and H lookup tables into GPU memory
        INT32 msElapsed = m_hrt.GetElapsed( false );
        loadR( &gi );
        loadHJ( &gi );

        // join all tuGpuP threads here
        m_pab->semLoadLUTs.Set();               // decrement the inverse-semaphore count
        m_pab->semLoadLUTs.Wait( INFINITE );    // wait for all tuGpuP threads to reach this point

        if( m_gpuDeviceOrdinal == 0 )
        {
            // performance metrics
            m_pab->DumpCudaGmemLayout( cdpCDb );
            AriocBase::aam.ms.InitializeGPUs += (m_hrt.GetElapsed(false) - msElapsed);

            // update progress
            m_pab->UpdateProgress( 0 );
        }
        ...
        */
        ...
        QBatchPool qbp( 2, m_pab, &gi, m_pab->pifgQ->GetEstimatedNmax() );

        // get an empty QBatch instance
        INT32 iPart = -1;                   // set up to start with the first input-file partition
        QBatch* pqb = qbp.Depool( m_pab->pifgQ->GetNextPartition( iPart ) );
        ...
        while( pqb->pfiQ )
        {
            ...
            while( qrp.LoadQ( pqb ) )
            {
                ...
                */
                if( tailP )
                {
                    msElapsed = hrt.GetElapsed( false );
        ...
        if( tailP )
        {
            msElapsed = m_hrt.GetElapsed( false );
        ...
        // join all tuGpuP threads here
        m_pab->semUnloadLUTs.Set();             // decrement the inverse-semaphore count
        m_pab->semUnloadLUTs.Wait( INFINITE );  // wait for all tuGpuP threads to reach this point

        // discard the current contents (if any) of the LUT buffers in CUDA global memory on the current GPU device
        unloadHJ( &gi );
        ...
        // performance metrics
        AriocBase::aam.cb.TotalCGA[gi.deviceId] = cga.cb;
        AriocBase::aam.cb.UsedCGA[gi.deviceId] = cga.cb - cga.cbMinFree;
