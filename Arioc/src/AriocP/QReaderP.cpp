    ...
    while( !QFI[0].eod )
    {
        ...
        */
        ...
        if( (readId >= QFI[0].pfi->ReadIdFrom) && (readId <= QFI[0].pfi->ReadIdTo) )
        {
            ...
            {
#ifdef _DEBUG
                // verify the maximum read length in the Qwarp
                if( pQw->Nmax > pqb->Nmax )
                    throw new ApplicationException( __FILE__, __LINE__, "%s: pQw->Nmax actual/expected = %d/%d", __FUNCTION__, pQw->Nmax, pqb->Nmax );
#endif
