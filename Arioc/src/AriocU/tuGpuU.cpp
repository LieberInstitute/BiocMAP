...
void tuGpuU::main()
{
    ...
    try
    {
        ...
        m_pab->semLoadLUTs.Wait( INFINITE );    // wait for all tuGpuU threads to reach this point
    ...
    CDPrint( cdpCD0, "%s: sqId 0x%016llx - 0x%016llx", __FUNCTION__, pQwFirst->sqId[0], pQwLast->sqId[pQwLast->nQ-1] );
        ...
        m_pab->semUnloadLUTs.Wait( INFINITE );  // wait for all tuGpuU threads to reach this point
