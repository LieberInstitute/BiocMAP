...
/// default constructor
OutputFileInfo::OutputFileInfo() : path(), rootPath(), relPath(), baseName(),   // (parentheses cause zero-initialization)
                                   maxA(0), oft(oftUnknown), arf(arfNone)
{
}

/// constructor
OutputFileInfo::OutputFileInfo( const char* _basePath, const char* _oftPath, const char* _baseName, OutputFormatType _oft, AlignmentResultFlags _arf, INT64 _maxA ) :
                                path(), rootPath(), relPath(), baseName(),   // (parentheses cause zero-initialization)
                                maxA(_maxA), oft(_oft), arf(_arf)
{
