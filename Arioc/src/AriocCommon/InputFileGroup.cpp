...
InputFileGroup::InputFileGroup( const char* filePath, const char* uriPath ) : m_partSeqNo(-1), InputFile(), HasPairs(false)
{
...
int InputFileGroup::fileInfoComparer( const void* a, const void* b )
{
    ...
    // order by srcId, subId, mateId, Filespec
    int rval = pa->SrcId - pb->SrcId;
    if( rval == 0 )
    {
...
/// [private] method estimateNmax
void InputFileGroup::estimateNmax( INT16 iFile )
{
    using namespace tinyxml2;

    FileInfo* pfi = this->InputFile.p + iFile;

    // open the .cfg file associated with the encoded sequence data
    char cfgFilespec[MAX_PATH] = { 0 };
    strcpy_s( cfgFilespec, sizeof cfgFilespec, pfi->Filespec );

    // replace the file type and extension with ".cfg"
    char* p = cfgFilespec + strlen( cfgFilespec );
    while( *(--p) != '$' );
    strcpy_s( p, 5, ".cfg" );

    // load the XML from the file
    XMLDocument doc;
    LoadXmlFile( &doc, cfgFilespec );

    // look for the maxN attribute of the PG element
    XMLElement* pelPG = doc.RootElement()->FirstChildElement( "PG" );
    if( pelPG )
        pfi->EstimatedNmax = pelPG->IntAttribute( "maxN" );
    if( pfi->EstimatedNmax )
        return;

    /* At this point we failed to extract the maximum read length from the .cfg file. */
...
/// </summary>
void InputFileGroup::Append( const char* fileSpec, INT32 srcId, UINT8 subId, INT64 readIdFrom, INT64 readIdTo, INT32 mateId, const char* uriPathForFile, const char* pSN )
{
    ...
    if( pUriPathForFile )
    {
        // allocate enough additional space in the buffer for a maximum-length URI string
        UriBuf.Realloc( UriBuf.n+FILENAME_MAX, true );
        char* puri = UriBuf.p + UriBuf.n;

        // build the complete URI path
        strcpy_s( puri, FILENAME_MAX, pUriPathForFile );
        RaiiDirectory::ChopTrailingPathSeparator( puri );
        strcat_s( puri, FILENAME_MAX, "/" );                // assume that a URI uses forward slashes
        strcat_s( puri, FILENAME_MAX, fileSpec );

        // resize the buffer to free unused space
        UINT32 cbAdd = static_cast<UINT32>(strlen(puri)) + 1;
        UriBuf.Realloc( UriBuf.n+cbAdd, false );

        // append to the list of offsets to the URI strings
        InputFile.p[InputFile.n].ofsURI = UriBuf.n;
        UriBuf.n += cbAdd;
    }

    // if no SN (sequence name) was associated with the current file, use a stringified srcId/subId combination
    char sSubId[32];
    if( pSN == NULL )
    {
        /* With the following string format, the rules of sprintf give us a subId in the 3 low-order digits, e.g.
            ...
            srcId subId  sSubId
                ...
                1     1  1001
        */
        sprintf_s( sSubId, sizeof sSubId, "%03lld", (1000*static_cast<INT64>(srcId))+subId );
        pSN = sSubId;
    }

    // save a copy of the specified SAM sequence name (SN) string
    UINT32 cbAdd = static_cast<UINT32>(strlen(pSN)) + 1;
    SnBuf.Realloc( SnBuf.n+cbAdd, true );
    char* psnBuf = SnBuf.p + SnBuf.n;
    strcpy_s( psnBuf, cbAdd, pSN );

    // append to the list of pointers to the SN strings
    InputFile.p[InputFile.n].ofsSN = SnBuf.n;
    SnBuf.n += cbAdd;
...
{
    // reorder the list by srcId, subId, mateId
    ...
    while( n < (InputFile.n-1) )
    {
        /* a pair of files contains paired reads if ...
            - adjacent files have the same srcId and subId, AND
            - the second adjacent file's mateId flag is set
        */
        if( (InputFile.p[n].SrcId == InputFile.p[n+1].SrcId) &&
            (InputFile.p[n].SubId == InputFile.p[n+1].SubId) &&
            InputFile.p[n+1].MateId )
        {
