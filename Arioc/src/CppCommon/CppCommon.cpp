...
/// [extern] UnduplicateSpaces
char* UnduplicateSpaces( char* p )
{
    char* pFrom = p;
    char* pTo = p;

    // we assume we need to trim leading and trailing spaces, too
    while( isspace(*pFrom) )
        ++pFrom;

    // excise redundant spaces
    while( *pFrom )
    {
        *(pTo++) = *(pFrom++);
    ...
    // we assume we need to trim trailing spaces, too
    if( isspace(pTo[-1]) )
        --pTo;
