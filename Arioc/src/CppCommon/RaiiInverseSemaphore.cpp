...
#ifdef _WIN32
/// constructor (INT32)
RaiiInverseSemaphore::RaiiInverseSemaphore( UINT32 initialCount ) : m_ev(true,(initialCount==0)), m_count(initialCount)
{
...
RaiiInverseSemaphore::RaiiInverseSemaphore( UINT32 initialCount ) : m_ev(true,(initialCount==0)), m_count(initialCount), m_mtx(false)
{
    /* The pthreads synchronization event object is initialized with:
        - manual reset
        - initial state = signalled if initial count is zero
