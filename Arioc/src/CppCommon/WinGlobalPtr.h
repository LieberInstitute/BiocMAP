...
/// </summary>
template <typename T> class WinGlobalPtr : public WinGlobalPtrBase
{
    private:
    ...
    public:
        ...
        virtual void Realloc( const size_t nElements, const bool zeroAllocatedMemory );
...
// default constructor
template <typename T> WinGlobalPtr<T>::WinGlobalPtr() : WinGlobalPtrBase(), m_inCppHeap(false), p(NULL)
{
}

/// constructor (size_t, bool): allocates a block of memory in the system heap, sized to contain the specified
///  number of elements of type T
template <typename T> WinGlobalPtr<T>::WinGlobalPtr( const size_t nElements, const bool zeroAllocatedMemory ) : WinGlobalPtrBase(nElements, zeroAllocatedMemory), m_inCppHeap(false), p(NULL)
{
    this->Realloc( nElements, zeroAllocatedMemory );
}

/// copy constructor (WinGlobalPtr<T>&)
template <typename T> WinGlobalPtr<T>::WinGlobalPtr( const WinGlobalPtr<T>& other ) : WinGlobalPtrBase(), m_inCppHeap(false), p(NULL)
{
...
template <typename T> const char* WinGlobalPtr<T>::GetCudaAllocationTypeName( void )
{
    return "host";
}

/// returns a string indicating the type of memory allocation (e.g., "pinned", "host")
template <typename T> void* WinGlobalPtr<T>::GetBufferPointer( void )
{
    return this->p;
}
