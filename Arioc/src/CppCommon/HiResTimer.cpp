...
// method GetElapsed( bool )
INT32 HiResTimer::GetElapsed( const bool restart )
{
    // get the current tick count
...
// method GetElapsed( bool, HiResTimerGranularity )
INT32 HiResTimer::GetElapsed( const bool _restart, const HiResTimerGranularity _granularity )
{
    // get the current tick count
    INT64 stopAt = getCurrent();

    INT64 s = ((stopAt - m_startAt) * m_granularity) / m_freq;

    if( _granularity != m_granularity )
    {
        if( _granularity == ms )
            s = (s+500) / 1000;     // convert us to ms
    }
    else
    {
        if( _granularity == us )
            s *= 1000;              // convert ms to us
    }
