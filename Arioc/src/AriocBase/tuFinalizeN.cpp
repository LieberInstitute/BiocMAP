...
#ifdef _DEBUG
        // verify the total number of mappings (see AriocP::tuAlignN::copyKernelResults66 or AriocU::tuAlignN::copyKernelResults22)
        const UINT32 nBRLEAs = m_pqb->HBn.BRLEA.n / m_pqb->celBRLEAperQ;
        if( m_pqb->HBn.nMapped != nBRLEAs )
            throw new ApplicationException( __FILE__, __LINE__, "BRLEA count mismatch: m_pqb->HBn.nMapped=%u nBRLEAs=%u", m_pqb->HBn.nMapped, nBRLEAs );
#endif
    }
