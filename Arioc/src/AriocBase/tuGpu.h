...
{
    protected:
        static const INT32 MAINLOOPJOIN_TIMEOUT = 2 * 60000;    // timeout for thread join in main loop
    ...
    private:
        size_t sniffJeol( CudaGmemLayout* _pcgl, UINT32* _pHostBuf );
        void loadLUT( GpuInfo* _pgi, CudaGmemLayout* _pcgl );
        void unloadLUT( CudaGmemLayout* _pcgl );
    
    protected:
        tuGpu( void );
        void loadR( GpuInfo* pgi );
        void loadHJ( GpuInfo* _pgi );
        void loadQ( QBatch* pqb );
        void unloadQ( QBatch* pqb );
        void unloadHJ( GpuInfo* _pgi );
