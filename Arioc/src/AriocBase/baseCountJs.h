...
class baseCountJs : public tuBaseS, public CudaLaunchCommon
{
    ...
    public:
        struct KernelConstants
        {
            ...
            INT16   deviceOrdinal;  // current GPU device ordinal

            KernelConstants()
            {
            }

            KernelConstants( QBatch* _pqb ) : maxJg(_pqb->pab->aas.ACP.maxJg),
                                                 seedWidth(_pqb->pab->a21hs.seedWidth),
                                                 seedMask(_pqb->pab->a21hs.seedMask),
                                                 hashBitsMask(_pqb->pab->a21hs.hashBitsMask),
                                                 sand(_pqb->pab->a21hs.seedWidth),
                                                 seedMaskShift(63-(3*_pqb->pab->a21hs.seedWidth)),
                                                 doCTconversion(_pqb->pab->a21hs.baseConvert == A21HashedSeed::bcCT),
                                                 sps(_pqb->pab->StrandsPerSeed),
                                                 deviceOrdinal(_pqb->pgi->deviceOrdinal)
            {
            }
        };
