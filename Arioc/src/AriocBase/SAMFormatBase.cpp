...
INT32 SAMFormatBase::initializer()
{
    /* The idea is that three consecutive symbols constitute the "context" for each position in
        ...
        in three 3-bit values, there are 512 possible contexts for s0.
...
/// [protected] method emitRNAMEforSubId
UINT32 SAMFormatBase::emitRNAMEforSubId( char* pbuf, INT16 subId )
{
    /* emit the subunit ID string (see SAMHDBuilder::getSN) */
    char* pSN = m_pqb->pab->SAMhdb.SN.p + m_pqb->pab->SAMhdb.ofsSN.p[subId];
    UINT8 cch = *reinterpret_cast<UINT8*>(pSN) + 1;     // length of string plus trailing tab character
    memcpy( pbuf, pSN+1, cch );
    return cch;
}
