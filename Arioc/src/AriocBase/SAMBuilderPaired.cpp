...
UINT32 SAMBuilderPaired::emitRNEXT( char* pbuf, INT64 sqId, QAI* pQAI )
{
    if( m_pQAIx->flags & qaiMapped )
    {
        ...
        return emitRNAMEforSubId( pbuf, m_pQAIx->subId );
    }
...
/// [protected] method emitc3
UINT32 SAMBuilderPaired::emitc3( char* pbuf, INT64 sqId, QAI* pQAI )
{
    // if there are no base quality scores, emit nothing
    if( !m_pqb->MFBq[0].buf.Count )
        return 0;

    // point to the metadata row header for the current base quality scores
    UINT32 iMate = pQAI->qid & 1;
    UINT32 iMetadata = pQAI->qid / 2;
    MetadataRowHeader* pmrh = reinterpret_cast<MetadataRowHeader*>(m_pqb->MFBq[iMate].buf.p + m_pqb->MFBq[iMate].ofs.p[iMetadata]);

    char* p = strcpy( pbuf, "c3:i:" ) + 5;
    p += u32ToString( p, pmrh->byteVal );
    *(p++) = '\t';
    return static_cast<UINT32>(p - pbuf);
}
...
INT64 SAMBuilderPaired::WriteRowPc( OutputFileBinding* pofb, INT32 iBuf, INT64 sqId, PAI* pPAI )
{
    m_YT = "YT:Z:CP\t";     // CP: concordant pair
    m_arf = pPAI->arf;

    /* This implementation ensures that both mates of a pair are written as adjacent rows.  We do not ensure that secondary
        mappings for the same pair are adjacent to their primary mappings, which would require a call to baseARowWriter->Lock
...
INT64 SAMBuilderPaired::WriteRowPd( OutputFileBinding* pofb, INT32 iBuf, INT64 sqId, PAI* pPAI )
{
...
INT64 SAMBuilderPaired::writeMatesUnpaired( OutputFileBinding* pofb, INT32 iBuf, INT64 sqId, PAI* pPAI, const char* pYT )
{
    m_arf = pPAI->arf;
    m_YT = pYT;
    m_fragmentLength = 0;

    // count the number of mates flagged for output
    INT64 nRows = __popcnt( pPAI->arf & (arfWriteMate1|arfWriteMate2) );
    if( nRows == 0 )
        return 0;
...
INT64 SAMBuilderPaired::WriteRowPr( OutputFileBinding* pofb, INT32 iBuf, INT64 sqId, PAI* pPAI )
{
    return writeMatesUnpaired( pofb, iBuf, sqId, pPAI, "YT:Z:RP\t" );   // RP: rejected pair (both mates mapped but did not meet criteria for concordant or discordant mapping)
}

/// [public] method WriteRowPu
INT64 SAMBuilderPaired::WriteRowPu( OutputFileBinding* pofb, INT32 iBuf, INT64 sqId, PAI* pPAI )
{
    return writeMatesUnpaired( pofb, iBuf, sqId, pPAI, "YT:Z:UP\t" );   // UP: unaligned pair (one or both mates did not map)
}
#pragma endregion
