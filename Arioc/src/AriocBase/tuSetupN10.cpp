...
void tuSetupN10::copyKernelResults()
{
    ...
    try
    {
        ...
        // performance metrics
        INT32 usElapsed = m_hrt.GetElapsed( true );
        InterlockedExchangeAdd( &AriocBase::aam.us.ProbeHJ, usElapsed );
        InterlockedExchangeAdd( &m_ptum->us.Launch, usElapsed );
    ...
    {
        pex->SetCallerExceptionInfo( __FILE__, __LINE__, "%s: GPU device ID %d (device ordinal %d)", __FUNCTION__, m_pqb->pgi->deviceId, m_pqb->pgi->deviceOrdinal );
        CRTHROW;
    }
}
