...
static __device__ __constant__ __align__(8) CudaGmemLayout cglHn;
static __device__ __constant__ __align__(8) CudaGmemLayout cglJn;
...
static inline __device__ void loadJinfo( UINT64* poJ, UINT32* pnJ,
                                         UINT64 i64lo, UINT64 i64m1, UINT64 i64m2, UINT64 i64hi, UINT64 i64n, const UINT32 sps )
{
    ...
    for( INT16 i=0; i<cckc.npos; ++i )
    {
        ...
        UINT64 hval = getV5( cglHn, hashKey );
        ...
        if( ofsJ )
        {
            ...
            nJ = (hval & HVALUE_MASK_NJ) ?      // if the J-list count is in the H value ...
                 ...
                 getV5( cglJn, ofsJ++ );        // ... otherwise, get the count from the first item in the J list and increment the J-list offset
...
static __global__ void tuSetupN10_Kernel( const UINT32                     nQw,         // in: total number of Q warps
                                        ...
                                        )
{
    ...
    if( cckc.doCTconversion )
    {
        ...
        loadJinfo( poJ, pnJ, i64lo, i64m1, i64m2, i64hi, i64n, sps );
        ...
        if( cckc.sps == 2 )
        {
            ...
            loadJinfo( poJ+1, pnJ+1, i64lo, i64m1, i64m2, i64hi, i64n, sps );
        }
    }
...
void tuSetupN10::initConstantMemory()
{
    ...
    CRVALIDATE = cudaMemcpyToSymbol( cglHn, &m_pab->cglHn, sizeof cglHn, 0, cudaMemcpyHostToDevice );
    CRVALIDATE = cudaMemcpyToSymbol( cglJn, &m_pab->cglJn, sizeof cglJn, 0, cudaMemcpyHostToDevice );
}
...
void tuSetupN10::launchKernel( dim3& d3g, dim3& d3b, UINT32 cbSharedPerBlock )
{
    ...
    // execute the kernel
    tuSetupN10_Kernel<<< d3g, d3b, cbSharedPerBlock >>>( m_pqb->QwBuffer.n,         // in: number of Qwarps
