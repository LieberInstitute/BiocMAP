...
static __device__ __constant__ INT16                        ccDeviceOrdinal;
                                    ...
                                    )
{
    ...
    // compute the offset of the UINT32 in which the specified Jvalue5 begins
    UINT64 ofs32 = (oj * 5) >> 2;

    // get the base address for the J lookup table
    const UINT32* __restrict__ pJ;

    if( cglJn.layoutType == cgltGPmem )
    {
        ...
        INT32 iPart = ofs32 / cglJn.celPerPartition;
        pJ = cglJn.part[iPart].p;

        // compute the offset relative to the start of the partition
        ofs32 -= cglJn.part[iPart].ofs;
    }
    else
    {
        ...
        */
        pJ = cglJn.part[ccDeviceOrdinal].p;
    }
...
static __global__ void baseLoadJn_KernelDf( const UINT64* const __restrict__ poJBuffer, // in: J-list offsets (one per seed)
                                          ...
                                          )
{
    ...
    */
    ...
    INT32 iSeed;
    UINT32 qid;
    INT32 pos = loadJ( s, subId, iSeed, qid, poJBuffer, npos, sps, Dq );
...
static __global__ void baseLoadJn_KernelD( const UINT64* const __restrict__ poJBuffer,  // in: J-list offsets (one per seed)
                                           const UINT32                     nJtotal,    // in: total number of J values
                                           const UINT32                     npos,       // in: number of seed positions per Q sequence
                                           const UINT32                     sps,        // in: 2: seeding from Q+ and Q-; 1: seeding from Q+ only
...
void baseLoadJn::initConstantMemory()
{
    ...
    CRVALIDATE = cudaMemcpyToSymbol( ccDeviceOrdinal, &m_pqb->pgi->deviceOrdinal, sizeof(INT16), 0, cudaMemcpyHostToDevice );

    // copy the J lookup table layout
...
void baseLoadJn::launchKernelDf( dim3& d3g, dim3& d3b, UINT32 cbSharedPerBlock )
{
    ...
    // execute the kernel
    baseLoadJn_KernelDf<<< d3g, d3b, cbSharedPerBlock >>>( m_pqb->DBj.oJ.p,         // in: per-seed J-list offsets
...
void baseLoadJn::launchKernelD( dim3& d3g, dim3& d3b, UINT32 cbSharedPerBlock )
{
    ...
    // execute the kernel
    baseLoadJn_KernelD<<< d3g, d3b, cbSharedPerBlock >>>( m_pqb->DBj.oJ.p,          // in: per-seed J-list offsets
