...
static __device__ __constant__ __align__(8) CudaGmemLayout                  cglHg;
static __device__ __constant__ __align__(8) CudaGmemLayout                  cglJg;
...
typedef void (*pfnHashProbe)( const INT16 i, const UINT64* const __restrict__ pQi, const INT16 N, UINT64*& poJ, UINT32*& pnJ, UINT32& rbits, const UINT32 tid, pfnHashQiKmer hashKmerF, pfnHashQiKmer hashKmerRC );
...
static inline __device__ UINT64 getV5( const CudaGmemLayout& cgl, const UINT64 ofs )
{
    UINT64 ofs32 = (ofs * 5) >> 2;          // compute the offset of the UINT32 in which the specified 5-byte value begins
    const UINT32* __restrict__ p;

    if( cgl.layoutType == cgltGPmem )
    {
        /* The pointer in the CudaPeerBufInfo list references a per-GPU partition
            (visible via "peer access") of the lookup table. */

        // point to the start of the table partition
        INT32 iPart = ofs32 / cgl.celPerPartition;
        p = cgl.part[iPart].p;

        // compute the offset relative to the start of the partition
        ofs32 -= cgl.part[iPart].ofs;
    }
    else
    {
        /* The pointer in the CudaPeerBufInfo list references either:
            - a copy of the entire lookup table in page-locked host ("pinned") memory, or
            - a copy of the entire lookup table in CUDA global memory on the current GPU device
        */
        p = cgl.part[cckc.deviceOrdinal].p;
    }

    // read the 8 bytes that contain the required 5-byte value
...
static inline __device__ void probeLUT( UINT64* poJ, UINT32* pnJ, UINT32& rbits, const UINT32 hashKey, const UINT32 tid )
{
    // get the 5-byte Hvalue for the hash key
    UINT64 hval = getV5( cglHg, hashKey );
    ...
    if( ofsJ )
    {
        ...
        nJ = (hval & HVALUE_MASK_NJ) ?  // if the J-list count is in the H value ...
                ...
                getV5( cglJg, ofsJ++ ); // ... otherwise, get the count from the first item in the J list and increment the J-list offset
    ...
    *pnJ = nJ;
    ...
    // set up the J-list buffer pointer and offset (see getV5())
    const UINT32* __restrict__ pJ;
    if( cglJg.layoutType == cgltGPmem )
    {
        INT32 iPart = ofsJ / cglJg.celPerPartition;
        pJ = cglJg.part[iPart].p;

        // compute the offset relative to the start of the partition
        ofsJ -= cglJg.part[iPart].ofs;
    }
    else
        pJ = cglJg.part[cckc.deviceOrdinal].p;
...
static inline __device__ void hashProbeF( const INT16 i, const UINT64* const __restrict__ pQi, const INT16 N, UINT64*& poJ, UINT32*& pnJ, UINT32& rbits, const UINT32 tid, pfnHashQiKmer hashKmerF, pfnHashQiKmer )
{
    // hash the forward kmer sequence
    UINT32 hash32 = hashQiKmerF( pQi, i, N );       // compute a 32-bit hash for the i'th kmer
    probeLUT( poJ++, pnJ++, rbits, hash32, tid );   // hash and count J values; accumulate rbits
}

/// [device] method hashProbeFRC
static inline __device__ void hashProbeFRC( const INT16 i, const UINT64* const __restrict__ pQi, const INT16 N, UINT64*& poJ, UINT32*& pnJ, UINT32& rbits, const UINT32 tid, pfnHashQiKmer hashKmerF, pfnHashQiKmer hashKmerRC )
{
    // hash the forward kmer sequence
    ...
    probeLUT( poJ++, pnJ++, rbits, hash32, tid );
}
                                         ...
                                         )
{
    ...
    */
    ...
    while( i <= iLimit )
    {
        ...
        hashProbe( i, pQi, pQw->N[iq], poJ, pnJ, rbits, tid, hashKmerF, hashKmerRC );
...
void baseCountJs::initConstantMemory()
{
    ...
    // copy the H and J lookup table layouts
    CRVALIDATE = cudaMemcpyToSymbol( cglHg, &m_pab->cglHg, sizeof cglHg, 0, cudaMemcpyHostToDevice );
    CRVALIDATE = cudaMemcpyToSymbol( cglJg, &m_pab->cglJg, sizeof cglJg, 0, cudaMemcpyHostToDevice );
}
