...
*/
#pragma once
#define __AriocAppMetrics__

/// <summary>
/// Class <c>AriocAppMetrics</c> records performance metrics.
/// </summary>
class AriocAppMetrics : public AriocMetricsBase
{
    ...
    public:
        ...
        struct
        {
            UINT64  AvailableCPU;           // number of bytes of physical host memory available to this process
            UINT64  TotalCGA[CUDAMAXGPUS];  // number of bytes of GPU memory managed by a CudaGlobalAllocator instance, per GPU
            UINT64  UsedCGA[CUDAMAXGPUS];   // maximum number of bytes of CGA-managed GPU memory used per GPU
            UINT64  Rpinned;                // number of bytes in pinned host memory buffer for R data
            UINT64  Rgmem;                  // number of bytes in CUDA global memory buffer for R data
            UINT64  Hnp;                    // number of bytes in pinned memory buffers for H table for nongapped aligner
            UINT64  Hgp;                    // number of bytes in pinned memory buffers for H table for gapped aligners
            UINT64  Jnp;                    // number of bytes in pinned memory buffers for J table for nongapped aligner
            UINT64  Jgp;                    // number of bytes in pinned memory buffers for J table for gapped aligner
            ...
            UINT64  Jng;					// number of bytes in CUDA global memory buffers for nongapped J table
            UINT64  Jgg;					// number of bytes in CUDA global memory buffers for gapped J table
            UINT64  HJpinned;               // number of bytes in pinned host memory buffers for H and J tables
			UINT64	HJgmem;					// number of bytes in CUDA global memory for H and J tables
            bool    HJpartitioned;          // true when H and J tables are partitioned across GPUs
        ...
        struct
        {
            ...
            UINT64  LoadHJ;                 // read H and J lookup tables from disk into CPU memory
            ...
            UINT64  WriteA;                 // write alignment output to disk

            UINT64  UnloadR;                // free R sequence data from memory
            UINT64  UnloadHJ;               // free lookup tables that reside in page-locked (pinned) CPU memory and/or CUDA global memory
        }
            ms;
        ...
        struct
        {
            ...
            UINT64  ProbeHJ;                // probe H and J lookup tables
        }
            us;
    ...
    private:
        ...
        void emitGpuMemInfo( CDPrintFlags cdp, const char* indent2, INT32 cchSpan1, INT32 cchSpan2 );
