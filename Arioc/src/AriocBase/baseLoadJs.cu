                                    ...
                                    )
{
    ...
    else
    {
        ...
        */
        pJ = cglJg.part[ccDeviceOrdinal].p;
    }

    // get the 5-byte J value
...
static __global__ void baseLoadJs_KernelD( const UINT64* const __restrict__ poJBuffer,  // in: J-list offsets (one per seed)
                                         ...
                                         )
{
    ...
    INT32 spos;
    UINT32 qid;
    INT32 pos = loadJ( s, subId, spos, qid, pQuBuffer, pRuBuffer, poJBuffer, sps, nSeedPos, D );
...
void baseLoadJs::initConstantMemory()
{
    ...
    // copy the current GPU device ordinal
...
void baseLoadJs::launchKernelD( dim3& d3g, dim3& d3b, UINT32 cbSharedPerBlock )
{
    ...
    // execute the kernel
    baseLoadJs_KernelD<<< d3g, d3b, cbSharedPerBlock >>>( m_pqb->DBj.oJ.p,          // in: per-seed J-list offsets
