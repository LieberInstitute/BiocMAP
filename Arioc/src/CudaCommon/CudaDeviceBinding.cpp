...
UINT32 CudaDeviceBinding::AvailableDeviceMap = 0;
UINT32 CudaDeviceBinding::ConfiguredDeviceMap = 0;
#ifdef _WIN32
...
CudaDeviceBinding::CudaDeviceBindingHelper::CudaDeviceBindingHelper(): m_psem(NULL)
{
    ...
    */
    ...
    CudaDeviceBinding::AvailableDeviceMap = (1 << nCudaDevices) - 1;
    CudaDeviceBinding::ConfiguredDeviceMap = CudaDeviceBinding::AvailableDeviceMap;
...
{
    CudaDeviceBinding::AvailableDeviceMap = 0x00000000;
    reallocateSemaphore();
}
...
void CudaDeviceBinding::CudaDeviceBindingHelper::reallocateSemaphore()
{
    ...
    int nCudaDevices = __popcnt( CudaDeviceBinding::AvailableDeviceMap );
    if( nCudaDevices )
    {
...
CudaDeviceBinding::CudaDeviceBinding( DWORD msTimeout ) : m_deviceId(-1), m_deviceMutex(false)
{
    ...
    // critical section: ensure that CPU threads have mutually-exclusive access to the bitmap of available CUDA devices
    {
        ...
        UINT8 rval = _BitScanForward( reinterpret_cast<DWORD*>(&m_deviceId), CudaDeviceBinding::AvailableDeviceMap );
        if( rval == 0 )
            throw new ApplicationException( __FILE__, __LINE__, "No available CUDA devices" );

        // zero the bit
        _bittestandreset( reinterpret_cast<LONG*>(&CudaDeviceBinding::AvailableDeviceMap), m_deviceId );
    }
...
CudaDeviceBinding::~CudaDeviceBinding()
{
    ...
    // critical section: ensure that CPU threads have mutually-exclusive access to the bitmap of available CUDA devices
    {
        ...
        _bittestandset( reinterpret_cast<LONG*>(&CudaDeviceBinding::AvailableDeviceMap), m_deviceId );
    }
...
void CudaDeviceBinding::initializeDevice( INT32 deviceId )
{
    CRVALIDATOR;
...
void CudaDeviceBinding::DeviceFilter_Reset()
{
    ...
    CudaDeviceBindingHelper* pcdbh = getHelperInstance();
    CudaDeviceBinding::AvailableDeviceMap = (1 << pcdbh->m_cudaDeviceProperties.Count) - 1;
    CudaDeviceBinding::ConfiguredDeviceMap = CudaDeviceBinding::AvailableDeviceMap;
...
void CudaDeviceBinding::DeviceFilter_MinGlobalMemory( size_t cbGlobalMemory )
{
    ...
    for( INT32 n=0; n<static_cast<INT32>(pcdbh->m_cudaDeviceProperties.Count); ++n )
    {
        ...
        {
            _bittestandreset( reinterpret_cast<LONG*>(&CudaDeviceBinding::AvailableDeviceMap), n );
            _bittestandreset( reinterpret_cast<LONG*>(&CudaDeviceBinding::ConfiguredDeviceMap), n );
        }
    }
...
/// </summary>
INT32 CudaDeviceBinding::GetAvailableDeviceCount()
{
    return __popcnt( CudaDeviceBinding::AvailableDeviceMap );
}

/// <summary>
/// Returns the number of configured CUDA devices
/// </summary>
INT32 CudaDeviceBinding::GetConfiguredDeviceCount()
{
    return __popcnt( CudaDeviceBinding::AvailableDeviceMap );
}
...
void CudaDeviceBinding::InitializeAvailableDevices()
{
    ...
    volatile DWORD dmap = CudaDeviceBinding::AvailableDeviceMap;
...
void CudaDeviceBinding::ResetAvailableDevices()
{
    ...
    // get a copy of the bitmap of available CUDA device IDs
    volatile UINT32 dmap = CudaDeviceBinding::AvailableDeviceMap;
...
/// <summary>
/// Determines whether all available GPU devices can directly access each other's memory
/// </summary>
bool CudaDeviceBinding::QueryPeerMemoryAccess()
{
    CRVALIDATOR;

    // determine whether all configured GPU devices are "peers" (i.e., if they can directly access each other's memory)
    bool allConfiguredDevicesArePeers = (__popcnt( CudaDeviceBinding::ConfiguredDeviceMap ) > 1);
    if( allConfiguredDevicesArePeers )
    {
        volatile UINT32 dmap = CudaDeviceBinding::ConfiguredDeviceMap;
        DWORD deviceId = -1;
        while( allConfiguredDevicesArePeers && _BitScanForward( &deviceId, dmap ) )
        {
            DWORD peerId = -1;
            volatile DWORD dmapPeer = CudaDeviceBinding::ConfiguredDeviceMap;
            while( allConfiguredDevicesArePeers && _BitScanForward( &peerId, dmapPeer ) )
            {
                if( deviceId != peerId )
                {
                    // determine whether device "deviceId" can access memory on device "peerId"
                    INT32 canAccessPeer = 0;
                    CRVALIDATE = cudaDeviceCanAccessPeer( &canAccessPeer, deviceId, peerId );
                    allConfiguredDevicesArePeers &= (canAccessPeer != 0);
                }

                _bittestandreset( const_cast<LONG*>(reinterpret_cast<volatile LONG*>(&dmapPeer)), peerId );
            }
    ...
    CDPrint( cdpCDb, "%s returns %d", __FUNCTION__, allConfiguredDevicesArePeers );
    return allConfiguredDevicesArePeers;
}

/// <summary>
/// Enable or disable peer memory access among all available GPU devices
/// </summary>
void CudaDeviceBinding::SetPeerMemoryAccess( bool _bEnable )
{
    CRVALIDATOR;

    volatile UINT32 dmap = CudaDeviceBinding::ConfiguredDeviceMap;
    DWORD deviceId = CudaDeviceBinding::GetCudaDeviceId();
    DWORD peerId = -1;
    while( _BitScanForward( &peerId, dmap ) )
    {
        if( peerId != deviceId )
        {
            if( _bEnable )
            {
                // enable memory accesses from the current GPU device to memory on device "peerId"
                cudaError_t rval = cudaDeviceEnablePeerAccess( peerId, 0 );
                if( rval != cudaSuccess )
                {
                    if( rval != cudaErrorPeerAccessAlreadyEnabled )
                        CRVALIDATE = rval;      // the usual error handling (i.e., throw an exception)
                    else
                        cudaGetLastError();     // clear the last error code
                }
            }
            else
            {
                // disable memory accesses from the current GPU device to memory on device "peerId"
                cudaError_t rval = cudaDeviceDisablePeerAccess( peerId );
                if( rval != cudaSuccess )
                {
                    if( rval != cudaErrorPeerAccessNotEnabled )
                        CRVALIDATE = rval;
                    else
                        cudaGetLastError();
                }
        ...
        _bittestandreset( const_cast<LONG*>(reinterpret_cast<volatile LONG*>(&dmap)), peerId );
    }
}
#pragma endregion
