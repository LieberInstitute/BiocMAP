...
#pragma region enums
enum CudaGmemLayoutType
{
    cgltUnknown =   0,
    cgltPinned =    1,  // pinned (page-locked host) memory
    cgltGmem =      2,  // CUDA global memory, copied to each GPU device
    cgltGPmem =     3   // CUDA global memory, partitioned across GPU devices
};
...
class CudaDeviceBinding
{
    ...
    public:
        static UINT32 AvailableDeviceMap;               // CUDA devices that are currently available
        static UINT32 ConfiguredDeviceMap;              // CUDA devices that may be used in this application
    ...
    public:
        ...
        static INT32 GetConfiguredDeviceCount( void );
        ...
        static bool QueryPeerMemoryAccess( void );
        void SetPeerMemoryAccess( bool _bEnable );
