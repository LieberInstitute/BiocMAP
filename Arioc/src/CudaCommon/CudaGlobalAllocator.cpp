...
/// default constructor
CudaGlobalAllocator::CudaGlobalAllocator( CudaDeviceBinding& cdb, INT64 cbReserved ) : p(NULL), cb(0), pLo(NULL), pHi(NULL), cbFree(0), cbMinFree(-1)
{
    CRVALIDATOR;

    // get available global memory for the CUDA device associated with the current CUDA context.
    INT32 deviceId = cdb.GetDeviceProperties()->cudaDeviceId;
    ...
    CDPrint( cdpCDb, "[%d] %s: initial cb=%lld", deviceId, __FUNCTION__, this->cb );
    ...
    this->cbFree = this->cb;
    this->cbMinFree = this->cbFree;
     
    CDPrint( cdpCDb, "[%d] %s: pLo=0x%016llx pHi=0x%016llx cb=%lld cbFree=%lld", deviceId, __FUNCTION__, this->pLo, this->pHi, this->cb, this->cbFree );
...
void* CudaGlobalAllocator::allocLo( INT64 cb )
{
    ...
    */
    INT64 cbNeeded = (cb ? round2power(cb, CudaGlobalAllocator::Granularity) : CudaGlobalAllocator::Granularity);
    ...
    if( this->cbFree < this->cbMinFree )
        this->cbMinFree = this->cbFree;         // track the "low water mark"
