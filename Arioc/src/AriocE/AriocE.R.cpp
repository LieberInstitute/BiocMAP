    ...
    */
    {
        ...
        while( n < nSq )
        {
            ...
            // conditionally save a URI for the nth input filename
            pscw->AppendSQAttribute( this->Params.ifgRaw.InputFile.p[i].SrcId,
                                     this->Params.ifgRaw.InputFile.p[i].SubId,
                                     "UR",
                                     this->Params.ifgRaw.UriBuf.p+this->Params.ifgRaw.InputFile.p[i].ofsURI );

            // conditionally save the SN for the nth input filename
            pscw->AppendSQAttribute( this->Params.ifgRaw.InputFile.p[i].SrcId,
                                     this->Params.ifgRaw.InputFile.p[i].SubId,
                                     "SN",
                                     this->Params.ifgRaw.SnBuf.p+this->Params.ifgRaw.InputFile.p[i].ofsSN );
        }
...
void AriocE::buildJlists()
{
    ...
    while( m < nSq )
    {
        ...
        // launch a worker thread for the ith input file
        ptuLoadR.p[m] = new tuLoadR( &this->Params, sqCatRplus, i, waitBits.p, &semEncoderWorkers );
        ptuLoadR.p[m++]->Start();
        ...
        ptuLoadR.p[m] = new tuLoadR( &this->Params, sqCatRminus, i, waitBits.p, &semEncoderWorkers );
        ptuLoadR.p[m++]->Start();
    }
...
size_t AriocE::compactJtable()
{
    ...
    */
    ...
    Jvalue5* pJ5 = pJ5base + 1;     // (element 0 of the J table is reserved (see struct JtableHeader))
...
void AriocE::validateHJ()
{
    ...
    // save the maximum J value in the 0th 5-byte element of the J-list buffer
...
void AriocE::ImportR()
{
    ...
    // sanity check: the reference sequence IDs should be in ascending order (see InputFileGroup::CleanMateIds())
    for( UINT32 n=1; n<this->Params.ifgRaw.InputFile.n; ++n )
    {
        INT32 srcIdA = this->Params.ifgRaw.InputFile.p[n-1].SrcId;
        INT32 srcIdB = this->Params.ifgRaw.InputFile.p[n].SrcId;
        UINT8 subIdA = this->Params.ifgRaw.InputFile.p[n-1].SubId;
        UINT8 subIdB = this->Params.ifgRaw.InputFile.p[n].SubId;
        INT64 sqIdA = AriocDS::SqId::PackSqId( subIdA, 0, 0, srcIdA );
        INT64 sqIdB = AriocDS::SqId::PackSqId( subIdB, 0, 0, srcIdB );
        if( sqIdA >= sqIdB )
            throw new ApplicationException( __FILE__, __LINE__, "input file source IDs and subunit IDs must be distinct and in ascending order (%d,%u does not precede %d,%u)", srcIdA, subIdA, srcIdB, subIdB );
    }
