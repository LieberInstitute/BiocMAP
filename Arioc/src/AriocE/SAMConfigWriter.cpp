...
tinyxml2::XMLElement* SAMConfigWriter::createSQ( const INT32 srcId, const UINT8 subId )
{
    ...
    // associate a "srcId" attribute with the SQ element
    elSQ->SetAttribute( "srcId", srcId );

    // associate a "subId" attribute with the SQ element
    char sSubId[8];
    sprintf_s( sSubId, sizeof sSubId, CONFIG_SUBID_FORMAT, subId );
    elSQ->SetAttribute( "subId", sSubId );
...
tinyxml2::XMLElement* SAMConfigWriter::getSQ( const INT32 srcId, const UINT8 subId )
{
    using namespace tinyxml2;
    ...
    // look for an SQ element with the specified srcId and subId as attributes
    ...
    if( elSQ == NULL )
    {
        ...
        XMLElement* elSQ = createSQ( srcId, subId );
        elSAM->InsertAfterChild( elHD, elSQ );
        return elSQ;
    }

    /* look for the specified subId in the set of existing SQ elements; for each srcId, the list is maintained
        in ascending order of subId */
    XMLElement* elPrev;
    do
    {
        INT32 existingSrcId = -1;
        if( (XML_NO_ERROR == elSQ->QueryIntAttribute( "srcId", &existingSrcId )) && (existingSrcId == srcId) )
        {
            const char* existingSubId = elSQ->Attribute( "subId" );
            switch( strcmp( existingSubId, sSubId ) )
            {
...
/// [public] method AppendSQAttribute
void SAMConfigWriter::AppendSQAttribute( const INT32 _srcId, const UINT8 _subId, const char* _attrName, const char* _attrValue )
{
    using namespace tinyxml2;
    ...
    if( _attrValue != NULL )
    {
        ...
        // get the SQ element
        XMLElement* elSQ = getSQ( _srcId, _subId );

        // add or update the specified attribute
        elSQ->SetAttribute( _attrName, _attrValue );
    }
}
...
/// [public] method AppendMaximumReadLength
void SAMConfigWriter::AppendMaximumReadLength( const INT16 maxN )
{
    using namespace tinyxml2;
    ...
    // add a maxN attribute
    char buf[8] = { 0 };
    sprintf_s( buf, sizeof buf, "%d", maxN );
    elPG->SetAttribute( "maxN", buf );
}
