...
void baseEncode::writeSqmR( INT64 readId, char* pRaw, INT64 cb )
{
    ...
    INT64 sqId = AriocDS::SqId::PackSqId( m_subId, m_isRminus, 0, m_srcId );
    ...
    *reinterpret_cast<INT16*>(p) = cbMetadata;  // length of metadata string
    ...
    if( !m_isRminus )
    {
        ...
        m_pscw->AppendSQAttribute( m_srcId, m_subId, "rm", pRaw );
    }
}
...
void baseEncode::writeSqmQ( INT64 readId, char* pRaw, INT64 cb )
{
    ...
    INT64 sqId = AriocDS::SqId::PackSqId( readId, m_pairFlag, m_subId, m_srcId );
...
void baseEncode::endRowRawR()
{
    ...
    *pcb = m_cbRawCurrent - (sizeof(INT64)+sizeof(INT64));
    ...
    if( !m_isRminus )
    {
        // emit the reference-sequence length as metadata
        char buf[24];
        sprintf_s( buf, sizeof buf, "%lld", *pcb );
        m_pscw->AppendSQAttribute( m_srcId, m_subId, "LN", buf );
    }
}
...
void baseEncode::writeConfigFile()
{
    ...
    if( m_psip->maxN )
        m_pscw->AppendMaximumReadLength( m_psip->maxN );
