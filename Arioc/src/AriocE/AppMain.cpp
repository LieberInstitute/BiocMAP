...
void AppMain::Launch()
{
    ...
    INT32 srcIdCommon = 0;                                                  // default value = 0
    m_pelDataIn->QueryIntAttribute( "srcId", &srcIdCommon );                // if "srcId" is not specified, the default value is used
    if( srcIdCommon > AriocDS::SqId::MaxSrcId )
        throw new ApplicationException( __FILE__, __LINE__, "invalid value %d for attribute srcId in element (valid values: 0-%d)", srcIdCommon, AriocDS::SqId::MaxSrcId );
    ...
    {
        INT32 srcId = srcIdCommon;                                      // if srcID is specified, it overrides the common data source ID
        pelFile->QueryIntAttribute( "srcId", &srcId );
        ...
        const char* pSN = pelFile->Attribute( "SN" );                   // if a "SN" attribute is specified, get a pointer to it
        ifgRaw.Append( pelFile->GetText(), srcId, subId, InputFileGroup::DefaultReadIdFrom, InputFileGroup::DefaultReadIdTo, mateId, uriPathForFile, pSN );
    ...
    for( INT32 n=0; n<nInputFiles; n++ )
    {
        ...
        CDPrint( cdpCD2, " input file %3d     : (%d.%u%s) %s", n, ifgRaw.InputFile.p[n].SrcId, ifgRaw.InputFile.p[n].SubId, buf, ifgRaw.InputFile.p[n].Filespec );
    }
    ...
    try
    {
        ...
        AriocE ae( ifgRaw, enc, pa21sb, outDir, maxDOP, samplingRatio, qualityScoreBias, emitKmers, gpuMask, pQNAME, maxJ, this );
